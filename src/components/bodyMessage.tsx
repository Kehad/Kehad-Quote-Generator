...
const BodyMessage: React.FC = () => {
  ...
  const copyToClipboard = async (): Promise<void> => {
    ...
    try {
      await navigator.clipboard.writeText(`"${quoteObject.quote}" — ${quoteObject.author}`);
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100 selection:bg-indigo-500/30 flex items-center justify-center p-6 font-sans">
      ...
      <div 
        ...
        style={{ background: `radial-gradient(circle at 50% 50%, ${accentColor} 0%, transparent 50%)` }}
      />
      ...
      <main className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <section className="md:col-span-3 flex flex-col md:flex-row justify-between items-end mb-4 px-2">
          <div>
            <h1 className="text-4xl font-light tracking-tight italic">thought<span className="font-bold not-italic text-indigo-400">flow</span></h1>
            <p className="text-slate-400 text-sm mt-1 uppercase tracking-widest">Curation 2.6</p>
          </div>
          <div className="flex gap-2 mt-4 md:mt-0">
             <div className="px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs font-medium backdrop-blur-sm">
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col h-full justify-between gap-8">
              <div className="space-y-6">
                <span className="text-6xl font-serif text-indigo-500/40 block h-8 leading-none">“</span>
                <p className="text-2xl md:text-4xl font-medium leading-tight tracking-tight text-slate-50">
                  {quoteObject?.quote}
                </p>
                <p className="text-lg text-slate-400 font-light">— {quoteObject?.author}</p>
              </div>
              ...
              <div className="flex items-center gap-4 pt-8 border-t border-white/5">
                ...
                >
                  {favorites.some(f => f.quote === quoteObject?.quote) ? '❤️' : '🤍'}
                </button>
              </div>
            </div>
          )}
        ...
        <aside className="space-y-4">
          <div className="rounded-[2rem] bg-white/[0.03] border border-white/10 p-6 backdrop-blur-md">
            ...
            >
              <span className="text-sm">{copyStatus === "success" ? "Copied!" : "Copy to Clipboard"}</span>
              <span className="opacity-40 group-hover:opacity-100 transition-opacity">↗</span>
            </button>
          </div>

          <div className="rounded-[2rem] bg-white/[0.03] border border-white/10 p-6 backdrop-blur-md">
            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-4">Settings</h3>
            <div className="space-y-4">
              <div>
                <label className="text-[10px] text-slate-500 uppercase block mb-2 font-bold">Language</label>
              ...
              <div>
                <label className="text-[10px] text-slate-500 uppercase block mb-2 font-bold">Curation</label>
                <select className="w-full bg-transparent text-sm border-b border-white/10 pb-1 focus:outline-none">
        ...
        <footer className="md:col-span-3 text-center py-6">
          <p className="text-[10px] text-slate-500 uppercase tracking-[0.2em]">
            Designed by <a href="https://www.linkedin.com/in/kehinde-adigun-/" className="text-indigo-400 hover:text-indigo-300 transition-colors">@Kehad</a>
          </p>
        </footer>
      </main>
    </div>
